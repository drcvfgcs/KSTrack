...
class DiMPActor_Decoder(BaseActor):
    """Actor for training the DiMP network."""
    def __init__(self, net, objective, loss_weight=None):
        super().__init__(net, objective)
        if loss_weight is None:
    ...
    def __call__(self, data):
        ...
        """
        # Run network
        target_scores, iou_pred, rec_pair = self.net(train_imgs=data['train_images'],
                                           test_imgs=data['test_images'],
                                           train_bb=data['train_anno'],
                                           test_proposals=data['test_proposals'])
        ...
        loss_rec = self.loss_weight['rec'] * self.objective['rec'](rec_pair[1::2,:,:,:],rec_pair[::2,:,:,:].detach())

        # loss_rec = self.loss_weight['rec'] * loss_rec
        ...
        # Total loss
        loss = loss_iou + loss_target_classifier + loss_test_init_clf + loss_test_iter_clf + loss_rec

        # Log stats
        stats = {'Loss/total': loss.item(),
                 'Loss/iou': loss_iou.item(),
                 'Loss/target_clf': loss_target_classifier.item(),
                 'Loss/rec_loss': loss_rec}
        if 'test_init_clf' in self.loss_weight.keys():
            stats['Loss/test_init_clf'] = loss_test_init_clf.item()
        if 'test_iter_clf' in self.loss_weight.keys():
            stats['Loss/test_iter_clf'] = loss_test_iter_clf.item()
...
class DiMPActor_DualDecoder(BaseActor):
    """Actor for training the DiMP network."""
    def __init__(self, net, objective, loss_weight=None):
        super().__init__(net, objective)
        if loss_weight is None:
    ...
    def __call__(self, data):
        ...
        """
        # Run network
        target_scores, iou_pred, rec_pair1, rec_pair2 = self.net(train_imgs=data['train_images'],
                                                                 test_imgs=data['test_images'],
                                                                 train_bb=data['train_anno'],
                                                                 test_proposals=data['test_proposals'])
        ...
        ## compute reconstruction loss
        loss_rec = self.loss_weight['rec'] * (self.objective['rec'](rec_pair1[1::2,:,:,:],rec_pair1[::2,:,:,:].detach()) + self.objective['rec'](rec_pair2[1::2,:,:,:],rec_pair2[::2,:,:,:].detach()))
