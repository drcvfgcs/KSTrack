...
class Siamese_DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.feat_decoder_rgb = nn.Conv2d(in_channels=256,out_channels=256,kernel_size=1)
        self.feat_decoder_tir = nn.Conv2d(in_channels=256,out_channels=256,kernel_size=1)

        nn.init.orthogonal_(self.feat_decoder_tir.weight)
        nn.init.orthogonal_(self.feat_decoder_rgb.weight)
    ...
    def extract_simease_backbone_feat(self,input_images):
        ...
        image_feats = {layer: torch.cat([rgb_feats[layer],tir_feats[layer]],axis=0) for layer in rgb_feats.keys()}
        input_feat = image_feats['layer3'].flatten(2).permute(0,2,1)
        output_feat = self.mixer(input_feat)
        output_feat = output_feat.permute(0,2,1).reshape(-1,256,18,18)
        image_feats['layer3'] = output_feat

        for layer, src in image_feats.items():
            bs, c, h, w = src.shape
            ind = int(bs/2)
            new = torch.cat([src[:ind,:,:,:],src[ind:,:,:]],axis=1)
            if layer == 'layer2':
                # temp = new.flatten(2)
                # u,s,v = torch.linalg.svd(temp)
                # image_feats[layer] = v[:,:128,:].view(-1,128,36,36)
                new = self.bn1(new)
                image_feats[layer] = self.feat_aggregator1(new)
            if layer == 'layer3':
                # temp = new.flatten(2)
                # u,s,v = torch.linalg.svd(temp)
                # image_feats[layer] = v[:,:256,:].view(-1,256,18,18)
                new = self.bn2(new)
                image_feats[layer] = self.feat_aggregator2(new)

        rec_rgb_feats = self.feat_decoder_rgb(image_feats['layer3'])
        rec_tir_feats = self.feat_decoder_tir(image_feats['layer3'])
    ...
    def extract_backbone_features(self,imgs, layers=None):
        ...
        tir_feats = self.extract_backbone_tir_features(tir_imgs.reshape(-1, *tir_imgs.shape[-3:]))
        feat_dict = {}
        feat_dict['layer2'] = self.bn1(torch.cat([rgb_feats['layer2'],tir_feats['layer2']],axis=1))
        feat_dict['layer3'] = self.bn2(torch.cat([rgb_feats['layer3'],tir_feats['layer3']],axis=1))
