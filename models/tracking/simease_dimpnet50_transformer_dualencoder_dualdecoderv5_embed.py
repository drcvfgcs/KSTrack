...
class Decoder(nn.Module):
    ...
    def forward(self, src, query_embed):
        ...
        return hs.permute(1,0,2).view(bs,-1,h,w)
...
class Siamese_DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.encoder = Encoder(d_model=324,nhead=6)
        self.encoder1 = Encoder(d_model=1296,nhead=6)
        ...
        self.rgb_placeholder = nn.Embedding(256, 324)
        self.tir_placeholder = nn.Embedding(256, 324)

        self.rgb_placeholder1 = nn.Embedding(128, 1296)
        self.tir_placeholder1 = nn.Embedding(128, 1296)


        ## decoder embedding placeholder
        self.query_embed_rgb = nn.Embedding(256, 324)
        self.query_embed_tir = nn.Embedding(256, 324)

        self.query_embed_rgb1 = nn.Embedding(128, 1296)
        self.query_embed_tir1 = nn.Embedding(128, 1296)

        nn.init.orthogonal_(self.query_embed_tir.weight)
        nn.init.orthogonal_(self.query_embed_rgb.weight)
        nn.init.orthogonal_(self.query_embed_tir1.weight)
        nn.init.orthogonal_(self.query_embed_rgb1.weight)
        nn.init.orthogonal_(self.rgb_placeholder.weight)
        nn.init.orthogonal_(self.rgb_placeholder1.weight)
        nn.init.orthogonal_(self.tir_placeholder.weight)
        nn.init.orthogonal_(self.tir_placeholder1.weight)

        self.decoder = Decoder(d_model=18*18,nhead=6)

        self.decoder1 = Decoder(d_model=36*36,nhead=6)
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        train_feat, train_rec_pair1, train_rec_pair2 = self.extract_simease_backbone_feat(train_imgs)
        test_feat, test_rec_pair1, test_rec_pair2 = self.extract_simease_backbone_feat(test_imgs)


        rec_pair1 = torch.cat([train_rec_pair1,test_rec_pair1])
        rec_pair2 = torch.cat([train_rec_pair2,test_rec_pair2])
        ...
        return target_scores, iou_pred, rec_pair1, rec_pair2
    ...
    def extract_simease_backbone_feat(self,input_images):
        ...
        rec_rgb_feats1 = self.decoder1(image_feats['layer2'],self.query_embed_rgb1.weight)
        rec_tir_feats1 = self.decoder1(image_feats['layer2'],self.query_embed_tir1.weight)

        # rec_err = self.mse_loss(rec_rgb_feats,rgb_feats['layer3']) + self.mse_loss(rec_tir_feats,rgb_feats['layer3'])
        rec_pair1 = torch.stack([rgb_feats['layer3'],rec_rgb_feats, tir_feats['layer3'], rec_tir_feats])
        rec_pair2 = torch.stack([rgb_feats['layer2'], rec_rgb_feats1, tir_feats['layer2'],rec_tir_feats1])
        # rec_err = self.mse_loss(rec_pair1[1::2,:,:,:],rec_pair1[::2,:,:,:].detach()) + self.mse_loss(rec_pair2[1::2,:,:,:],rec_pair2[::2,:,:,:].detach())

        return image_feats, rec_pair1, rec_pair2


    def replace_feature(self,rgb_feats, tir_feats):
        '''
        replace modal feature with query embedding
        '''
        modal = np.random.choice([0,1],1)

        ## replace rgb feature
        if modal == 0:
            replaced_rgb = {}
            bs,c,h,w = rgb_feats['layer3'].shape
            temp_rgb = self.rgb_placeholder.weight.view([c,h,w])
            replaced_rgb['layer3'] = temp_rgb.expand([bs,c,h,w])

            bs,c,h,w = rgb_feats['layer2'].shape
            temp_rgb = self.rgb_placeholder1.weight.view([c,h,w])
            replaced_rgb['layer2'] = temp_rgb.expand([bs,c,h,w])
            image_feats = {layer: torch.cat([replaced_rgb[layer],tir_feats[layer]],axis=1) for layer in rgb_feats.keys()}

        ## replace tir feature
        if modal == 1:
            replaced_tir = {}
            bs,c,h,w = tir_feats['layer3'].shape
            temp_tir = self.tir_placeholder.weight.view([c,h,w])
            replaced_tir['layer3'] = temp_tir.expand([bs,c,h,w])

            bs,c,h,w = tir_feats['layer2'].shape
            temp_tir = self.tir_placeholder1.weight.view([c,h,w])
            replaced_tir['layer2'] = temp_tir.expand([bs,c,h,w])
            image_feats = {layer: torch.cat([rgb_feats[layer],replaced_tir[layer]],axis=1) for layer in rgb_feats.keys()}
    ...
    def extract_backbone_features(self,imgs, layers=None):
        ...
        # bs,c,h,w = rgb_feats['layer3'].shape
        # temp_tir = self.tir_placeholder.weight.view([c,h,w])
        # tir_feats['layer3'] = temp_tir.expand([bs,c,h,w])
        #
        # bs,c,h,w = rgb_feats['layer2'].shape
        # temp_tir = self.tir_placeholder1.weight.view([c,h,w])
        # tir_feats['layer2'] = temp_tir.expand([bs,c,h,w])
        #
        ...
        # feat_dict['layer2'] = self.encoder1(feat_dict['layer2'])
        # feat_dict['layer3'] = self.feat_aggregator2(feat_dict['layer3'])
        # feat_dict['layer2'] = self.feat_aggregator1(feat_dict['layer2'])
        # rec_feat1 = self.decoder(feat_dict['layer3'],self.query_embed_tir.weight)
        # tir_feats['layer3'] = rec_feat1
        # rec_feat2 = self.decoder1(feat_dict['layer2'],self.query_embed_tir1.weight)
        # tir_feats['layer2'] = rec_feat2

        ## only tir
        # rgb_feats = {}
        # bs,c,h,w = tir_feats['layer3'].shape
        # temp_rgb = self.tir_placeholder.weight.view([c,h,w])
        # rgb_feats['layer3'] = temp_rgb.expand([bs,c,h,w])
        #
        # bs,c,h,w = tir_feats['layer2'].shape
        # temp_rgb = self.tir_placeholder1.weight.view([c,h,w])
        # rgb_feats['layer2'] = temp_rgb.expand([bs,c,h,w])
        #
        # feat_dict = {layer: torch.cat([rgb_feats[layer],tir_feats[layer]],axis=1) for layer in rgb_feats.keys()}
        # feat_dict['layer3'] = self.encoder(feat_dict['layer3'])
        # feat_dict['layer3'] = self.feat_aggregator2(feat_dict['layer3'])
        # rec_feat = self.decoder(feat_dict['layer3'],self.query_embed_rgb.weight)
        # rgb_feats['layer3'] = rec_feat
        # rec_feat2 = self.decoder1(feat_dict['layer2'],self.query_embed_rgb1.weight)
        # rgb_feats['layer2'] = rec_feat2
