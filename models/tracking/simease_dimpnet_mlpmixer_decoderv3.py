...
class Siamese_DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.feat_decoder_rgb = nn.Linear(in_features=256,out_features=256,bias=True)
        self.feat_decoder_tir = nn.Linear(in_features=256,out_features=256,bias=True)
    ...
    def extract_simease_backbone_feat(self,input_images):
        ...
        bs, c, h, w = image_feats['layer3'].shape
        rec_rgb_feats = self.feat_decoder_rgb(image_feats['layer3'].flatten(2).permute(0,2,1)).view(bs,c,h,w)
        rec_tir_feats = self.feat_decoder_tir(image_feats['layer3'].flatten(2).permute(0,2,1)).view(bs,c,h,w)
