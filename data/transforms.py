...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            ...
            # img_gray1 = cv.cvtColor(image[:,:,:3], cv.COLOR_RGB2GRAY)
            # img_gray2 = cv.cvtColor(image[:,:,3:], cv.COLOR_RGB2GRAY)
            # img_gray = (img_gray1+img_gray2)/2
            # img_gray = img_gray.astype(np.int32)
            # return np.stack([img_gray1, img_gray2, img_gray], axis=2)
